...
export async function POST(request: NextRequest) {
  ...
  // GATED (hardening gen 12): dispute locks freeze a vault's balances — an
  // operator action. The signed admin cookie is verified before any body is
  // parsed.
  ...
  const result = await applyDisputeLock(getStore(), parsed.value);
  if (!result.ok) {
