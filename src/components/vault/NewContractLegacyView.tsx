/**
 * The legacy vault draft flow for /contracts/new — factory-library template
 * keys generating a draft through the vault's ContractEditor with SDK
 * asset hydration. The universal execution lane owns the aliased and atomic
 * registry keys on the same route; this view preserves the vault's
 * generation flow for the factory draft keys the vault links to.
 *
 * Restored verbatim from the pre-lane page (b1a83be) as a component so the
 * route can dispatch between the two flows.
 */
...
export default async function NewContractLegacyView({
  templateId,
  cbt,
}: {
  templateId: string;
  ...
  // The route dispatches: a key the vault library does not know falls back
  // to the lane or redirects — never an invented page here either.
